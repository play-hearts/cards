...
export function rankOrd(rank: RankName): number {
    if (!isRankName(rank))
        throw new Error(`Invalid RankName: ${rank}`);
...
export function isCard(card: unknown): card is CardType {
    return isSuitName((card as CardType).suit) && isRankName((card as CardType).rank);
}
...
export function isTrickCard(card: unknown): card is TrickCard {
    return ((card as TrickCard).suit === "" && (card as TrickCard).rank === "")
        || ((isSuitName((card as TrickCard).suit) && isRankName((card as TrickCard).rank)));
}
