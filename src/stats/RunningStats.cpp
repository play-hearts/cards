...
double RunningStats::mean() const { return N == 0 ? kNaN : M1; }

double RunningStats::variance() const { return N <= 1 ? kNaN : M2 / (N - 1.0); }

double RunningStats::stddev() const { return std::sqrt(variance()); }

double RunningStats::skewness() const { return N <= 2 ? kNaN : std::sqrt(double(N)) * M3 / pow(M2, 1.5); }

double RunningStats::kurtosis() const { return N <= 3 ? kNaN : N * M4 / (M2 * M2) - 3.0; }
...
RunningStats operator+(const RunningStats& a, const RunningStats& b)
{
    ...
    combined.M4
        += 6.0 * delta2 * (aN * aN * b.M2 + bN * bN * a.M2) / (cN * cN) + 4.0 * delta * (aN * b.M3 - bN * a.M3) / cN;
...
std::ostream& operator<<(std::ostream& stream, const RunningStats& stats)
{
    ...
    {
        return stream << stats.mean() << "+/-" << stats.stddev() << " skew:" << stats.skewness()
                      << " kurt:" << stats.kurtosis() << " min:" << stats.minimum() << " max:" << stats.maximum()
                      << " N:" << stats.samples();
    }
}
...
std::string toString(const RunningStats& stats)
{
    ...
    return fmt::format("{:.2f}+/-{:.2f} [{:.2f}, {:.2f}] sk:{:.2f} kr:{:.2f} N:{}", stats.mean(), s, min, max,
        stats.skewness(), stats.kurtosis(), stats.samples());
}
